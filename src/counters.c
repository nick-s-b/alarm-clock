...
void
fill_default_for_counter(gchar *alarm_name, gulong date)
{
	...
	g_key_file_set_string(key, alarm_name, "PassivePopupText", _("Short popup"));
	g_key_file_set_boolean(key, alarm_name, "DialogWindowEnabled", FALSE);
	g_key_file_set_string(key, alarm_name, "DialogTextColor", "#000000000000");
	g_key_file_set_string(key, alarm_name, "DialogBackgroundColor", "#ffffffffffff");
	g_key_file_set_integer(key, alarm_name, "DialogTextSize", 1);
	g_key_file_set_string(key, alarm_name, "DialogTitle", _("Dialog window"));
	g_key_file_set_string(key, alarm_name, "DialogMessage", _("This is a small message"));
...
void
add_counter_dialog(void)
{
	GtkWidget *dialog = GTK_WIDGET (gtk_builder_get_object (gxml, "counter_editor_dialog"));
	GtkWidget *minute_spin = GTK_WIDGET (gtk_builder_get_object (gxml, "minutes_spin"));
	GtkWidget *title_entry = GTK_WIDGET (gtk_builder_get_object (gxml, "title_entr"));
	GtkWidget *counter_text = GTK_WIDGET (gtk_builder_get_object (gxml, "counter_text"));
	GtkWidget *standard_sound_radio = GTK_WIDGET (gtk_builder_get_object (gxml, "standard_sound_radio"));
	GtkWidget *soundfile_chooser = GTK_WIDGET (gtk_builder_get_object (gxml, "soundfile_chooser"));
	GtkTextBuffer *buf = gtk_text_view_get_buffer(GTK_TEXT_VIEW(counter_text));

	GtkFileFilter *sound_filter = gtk_file_filter_new();
	GtkFileFilter *all_filter = gtk_file_filter_new();

	gtk_file_filter_add_mime_type(GTK_FILE_FILTER(sound_filter), "audio/*");
	gtk_file_filter_set_name(GTK_FILE_FILTER(sound_filter), _("Sound files"));
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(standard_sound_radio), TRUE);

	gtk_file_chooser_set_uri(GTK_FILE_CHOOSER(soundfile_chooser), "");
	gtk_file_chooser_add_filter(GTK_FILE_CHOOSER(soundfile_chooser), sound_filter);
	gtk_file_chooser_add_filter(GTK_FILE_CHOOSER(soundfile_chooser), all_filter);

	gtk_widget_show(GTK_WIDGET(dialog));
}

void
hide_counter_editor(void)
{
	GtkWidget *dialog = GTK_WIDGET (gtk_builder_get_object (gxml, "counter_editor_dialog"));
...
sound_toggled(void)
{
	GtkWidget *soundfile_radio = GTK_WIDGET (gtk_builder_get_object (gxml, "soundfile_radio"));
	GtkWidget *soundfile_chooser = GTK_WIDGET (gtk_builder_get_object (gxml, "soundfile_chooser"));

	gtk_widget_set_sensitive(GTK_WIDGET(soundfile_chooser), gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(soundfile_radio)));
}
...
counter_accept(void)
{
	...
	GtkWidget *counter_text = GTK_WIDGET (gtk_builder_get_object (gxml, "counter_text"));
	GtkWidget *soundfile_radio = GTK_WIDGET (gtk_builder_get_object (gxml, "soundfile_radio"));
	GtkWidget *no_sound_radio = GTK_WIDGET (gtk_builder_get_object (gxml, "no_sound_radio"));
	GtkWidget *standard_sound_radio = GTK_WIDGET (gtk_builder_get_object (gxml, "standard_sound_radio"));
	gchar *file = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(soundfile_chooser));
	key = g_key_file_new();
	GtkTextBuffer *contents_buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(counter_text));
	GtkTextIter start, end;
	GKeyFile *old_key = g_key_file_new();
	gint mins, number;
	gchar *alarm_name, *title_buffer, **keys, *counter_text_buffer, *keydata;
	gint x;
	...
	gboolean is_chosen = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(soundfile_radio));

	title_buffer = (gchar*)gtk_entry_get_text(GTK_ENTRY(title_entry));

	if (g_strcmp0(file, NULL) == 0 && is_chosen)
	{
		GtkWidget *message = gtk_message_dialog_new_with_markup (GTK_WINDOW(dialog),
	...
	if (g_strcmp0(title_buffer, "") == 0)
	{
		...
		gtk_message_dialog_set_markup(GTK_MESSAGE_DIALOG(message), _("<b>Invalid title</b>"));
		gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(message), _("You need to specify title of the counter."));
	...
	gtk_text_buffer_get_start_iter(GTK_TEXT_BUFFER(contents_buffer), &start);
	...
	counter_text_buffer = gtk_text_buffer_get_text(GTK_TEXT_BUFFER(contents_buffer), &start, &end, FALSE);
	...
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(standard_sound_radio)))
	{
		g_key_file_set_boolean(key, alarm_name, "SoundEnabled", TRUE);
		g_key_file_set_boolean(key, alarm_name, "CustomSoundEnabled", FALSE);
		g_key_file_set_string(key, alarm_name, "CustomSoundFile", "[DEFAULT]");
		g_key_file_set_boolean(key, alarm_name, "ConstantSoundEnabled", TRUE);
		g_key_file_set_integer(key, alarm_name, "ConstantSoundValue", 1);
	}
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(soundfile_radio)))
	{
		g_key_file_set_boolean(key, alarm_name, "SoundEnabled", TRUE);
		g_key_file_set_boolean(key, alarm_name, "CustomSoundEnabled", TRUE);
	...
	g_key_file_set_string(key, alarm_name, "PassivePopupTitle", title_buffer);
	g_key_file_set_string(key, alarm_name, "PassivePopupText", counter_text_buffer);
	...
	while (TRUE)
	{
		if (keys[x] == NULL) break;
		title_buffer = g_key_file_get_string(key, alarm_name, keys[x], NULL);
		g_key_file_set_string(old_key, alarm_name, keys[x], title_buffer);
		g_free(title_buffer);
		x++;
	}
