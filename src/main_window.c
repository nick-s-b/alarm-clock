...
void check_missed_alarms(void)
{
	...
	if (num_groups > 0) {
		...
		GtkWidget *old_alarms_dialog =
		    gtk_message_dialog_new_with_markup(GTK_WINDOW(main_window),
						       GTK_DIALOG_DESTROY_WITH_PARENT,
						       GTK_MESSAGE_WARNING,
						       GTK_BUTTONS_CLOSE, NULL);

		gtk_message_dialog_set_markup(GTK_MESSAGE_DIALOG
					      (old_alarms_dialog),
					      _("<b>You have old alarms</b>"));
...
{
	GtkWidget *menubar =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "menubar1"));
	show_menu = !show_menu;
	if (show_menu) {
...
GtkWidget *create_main_window(void)
{
	...
	GtkWidget *ampm_container =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "alignment16"));
	...
	gtk_tree_view_column_pack_start(templates_column, templates_renderer,
					FALSE);

	gtk_tree_view_column_add_attribute(GTK_TREE_VIEW_COLUMN
					   (templates_column),
					   templates_renderer, "text", 0);

	gtk_tree_view_append_column(GTK_TREE_VIEW(templates_tree),
				    templates_column);
	...
	gtk_tree_view_column_set_expand(GTK_TREE_VIEW_COLUMN(birthdays_column),
					TRUE);

	gtk_tree_view_column_pack_start(birthdays_column, birthdays_renderer,
					FALSE);
	gtk_tree_view_column_add_attribute(GTK_TREE_VIEW_COLUMN
					   (birthdays_column),
					   birthdays_renderer, "text", 0);

	gtk_tree_view_column_pack_start(birthdays_date_column,
					birthdays_date_renderer, FALSE);
	gtk_tree_view_column_add_attribute(GTK_TREE_VIEW_COLUMN
					   (birthdays_date_column),
					   birthdays_date_renderer, "text", 1);

	gtk_tree_view_append_column(GTK_TREE_VIEW(birthdays_tree),
				    birthdays_column);
	gtk_tree_view_append_column(GTK_TREE_VIEW(birthdays_tree),
				    birthdays_date_column);

	gtk_tree_view_column_set_title(GTK_TREE_VIEW_COLUMN(birthdays_column),
				       _("Name"));
	gtk_tree_view_column_set_title(GTK_TREE_VIEW_COLUMN
				       (birthdays_date_column), _("Date"));
	...
	GtkWidget *missed_tree =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "missed_alarms_tree"));

	missed_store = gtk_list_store_new(2, G_TYPE_STRING, G_TYPE_STRING);

	gtk_tree_view_set_model(GTK_TREE_VIEW(missed_tree),
				GTK_TREE_MODEL(missed_store));
	...
	gtk_tree_view_column_add_attribute(GTK_TREE_VIEW_COLUMN(missed_column),
					   missed_renderer, "text", 0);
	...
	schedule_dates_store =
	    gtk_tree_store_new(4, G_TYPE_STRING, G_TYPE_BOOLEAN, G_TYPE_INT,
			       G_TYPE_INT);

	gtk_tree_view_set_model(GTK_TREE_VIEW(schedule_dates_tree),
	...
	gtk_tree_view_column_pack_start(schedule_date_column,
					schedule_date_renderer, FALSE);
	gtk_tree_view_column_add_attribute(GTK_TREE_VIEW_COLUMN
					   (schedule_date_column),
					   schedule_date_renderer, "markup", 0);

	gtk_tree_view_append_column(GTK_TREE_VIEW(schedule_dates_tree),
				    schedule_date_column);

	gtk_tree_view_column_set_expand(GTK_TREE_VIEW_COLUMN
					(schedule_date_column), TRUE);
	gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(schedule_dates_tree),
					  TRUE);
	...
	gtk_combo_box_append_text(GTK_COMBO_BOX(text_size_combo),
				  _("Very large"));
	...
	posx =
	    g_key_file_get_integer(config, "WindowGeometry", "WindowPositionX",
				   NULL);
	posy =
	    g_key_file_get_integer(config, "WindowGeometry", "WindowPositionY",
				   NULL);
	width =
	    g_key_file_get_integer(config, "WindowGeometry", "WindowSizeX",
				   NULL);
	height =
	    g_key_file_get_integer(config, "WindowGeometry", "WindowSizeY",
				   NULL);
	...
	show_menu =
	    g_key_file_get_boolean(config, "Global", "ShowMenuBar", NULL);
	...
	gtk_tree_view_column_pack_end(main_column, name_renderer, FALSE);
	...
	GtkWidget *pri_hbox =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "hbox25"));
...
void check_for_updates(void)
{
	g_spawn_command_line_async
	    ("xdg-open https://github.com/shaggytwodope/alarm-clock/releases",
	     NULL);
}
