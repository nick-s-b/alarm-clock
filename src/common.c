...
void quit_alarm_clock(void)
{
	...
	g_key_file_set_integer(config, "WindowGeometry", "WindowPositionX",
			       posx);
	g_key_file_set_integer(config, "WindowGeometry", "WindowPositionY",
			       posy);
...
void about_alarm_clock(void)
{
	...
	gtk_about_dialog_set_copyright(GTK_ABOUT_DIALOG(about),
				       "(c) 2008-2010 Tomasz Sałaciński <tsalacinskI@gmail.com>\n(c) 2016-2017 Johnathan Jenkins <twodopeshaggy@gmail.com>");
	gtk_about_dialog_set_comments(GTK_ABOUT_DIALOG(about),
				      _("A simple alarm clock."));
	...
	gtk_about_dialog_set_website(GTK_ABOUT_DIALOG(about),
				     "https://github.com/shaggytwodope/alarm-clock");
...
void show_missed_alarms(void)
{
	GtkWidget *dialog =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "missed_alarms_dialog"));
	GtkWidget *notebook =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "missed_alarm_notebook"));
	GtkWidget *mbutton =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "clear_missed_button"));
	...
	if (num_groups > 0) {
		...
		for (x = 0; x < num_groups; x++) {
			buffer =
			    g_key_file_get_string(key, groups[x], "Title",
						  NULL);
			gtk_list_store_append(GTK_LIST_STORE(missed_store),
					      &iter);
			gtk_list_store_set(GTK_LIST_STORE(missed_store), &iter,
					   0, buffer, 1, groups[x], -1);
