...
void
volume_fade_thread(gchar *name)
{
	g_object_set(G_OBJECT(play), "volume", 0.0, NULL);
	gfloat current_volume = 0.0;
	gboolean repeat = g_key_file_get_boolean(loaded_alarms, name, "RepeatSoundEnabled", NULL);
	gint repeat_value = g_key_file_get_integer(loaded_alarms, name, "RepeatSoundValue", NULL);
	gfloat addition = 0.0;
	if (repeat)
		addition = 1.1 / (count_play - current_time.tv_sec - repeat_value * 60);
	else
...
static gboolean
constant_bus_callback (GstBus *bus,
			  GstMessage *message,
			  gpointer data)
{
	g_get_current_time(&current_time);
...
void
start_sound(gpointer user_data)
{
	if (sound_ref > 0)
	{
		gdk_threads_enter();
	...
	if (g_key_file_get_boolean(loaded_alarms, name, "CustomSoundEnabled", NULL))
	{
		soundfile = g_strdup_printf("file://%s", g_key_file_get_string(loaded_alarms, name, "CustomSoundFile", NULL));
	}
	else
	{
		soundfile = g_strdup_printf("file://%s", ALARM_CLOCK_DEFAULT_SOUND);
	}
	...
	{
		fade_enabled = TRUE;
		count_play = g_key_file_get_integer(loaded_alarms, name, "FadeSoundValue", NULL);
		count_play = count_play * 60 + current_time.tv_sec;
		gboolean repeat = g_key_file_get_boolean(loaded_alarms, name, "RepeatSoundEnabled", NULL);
		gint repeat_value = g_key_file_get_integer(loaded_alarms, name, "RepeatSoundValue", NULL);
...
void
show_popup(gchar *name)
{
	gint type, timeout;
	gchar *title, *text;
	
	type = g_key_file_get_integer(loaded_alarms, name, "PassivePopupType", NULL);
	timeout = g_key_file_get_integer(loaded_alarms, name, "PassivePopupTimeout", NULL);
	title = g_key_file_get_string(loaded_alarms, name, "PassivePopupTitle", NULL);
...
void
close_main_dialog (GtkButton *button, gpointer user_data)
{
...
void
enable_snooze(GtkWidget *button, gpointer user_data)
{
	GTimeVal before_time;
	GtkWidget *dialog = gtk_widget_get_toplevel(GTK_WIDGET(button));
	gchar *alarm_name = (gchar*)user_data;
	gint snooze_time = g_key_file_get_integer(loaded_alarms, alarm_name, "SnoozeTime", NULL);
	gchar *date = g_key_file_get_string(loaded_alarms, alarm_name, "DateTime", NULL);
	gchar *new_date;
	...
	g_key_file_set_string(loaded_alarms, alarm_name, "SnoozedOldTime", new_date);
...
show_dialog(gchar *name)
{
	...
	gboolean snooze = g_key_file_get_boolean(loaded_alarms, name, "SnoozeEnabled", NULL);
		
	if (snooze) 
	{
		running_snooze = TRUE;
		GtkWidget *snooze_button = gtk_dialog_add_button (GTK_DIALOG (dialog), _("Snooze"), GTK_RESPONSE_OK);
		g_signal_connect (G_OBJECT (snooze_button), "clicked",
						  G_CALLBACK (enable_snooze), g_strdup(name));		
	}
	
	
	gbuffer = g_key_file_get_string(loaded_alarms, name, "DialogTitle", NULL);
	...
	gbuffer = g_key_file_get_string(loaded_alarms, name, "DialogBackgroundColor", NULL);
	gdk_color_parse(gbuffer, &background_color);
	gtk_widget_modify_bg(GTK_WIDGET(event), GTK_STATE_NORMAL, &background_color);
	g_free(gbuffer);
	
	gbuffer = g_key_file_get_string(loaded_alarms, name, "DialogMessage", NULL);

	if (g_key_file_get_integer(loaded_alarms, name, "DialogTextSize", NULL) == 0)
		to_enter = g_strdup_printf("<small>%s</small>", gbuffer);

	else if (g_key_file_get_integer(loaded_alarms, name, "DialogTextSize", NULL) == 1)
		to_enter = g_strdup_printf("%s", gbuffer);

	else if (g_key_file_get_integer(loaded_alarms, name, "DialogTextSize", NULL) == 2)
		to_enter = g_strdup_printf("<big>%s</big>", gbuffer);

	else if (g_key_file_get_integer(loaded_alarms, name, "DialogTextSize", NULL) == 3)
	...
	gbuffer = g_key_file_get_string(loaded_alarms, name, "DialogTextColor", NULL);
...
void
show_command(gchar *name)
{
	gchar *gbuffer = "", *command = "";
	gboolean bool_buffer;

	bool_buffer = g_key_file_get_boolean(loaded_alarms, name, "RunInTerminal", NULL);
	gbuffer = g_key_file_get_string(loaded_alarms, name, "RunCommandContent", NULL);
	if (bool_buffer)
	{
		command = g_strdup_printf("%s; read -p \"%s\"; rm -fr /tmp/ac_script.sh", gbuffer, _("Press any key to continue..."));
		g_file_set_contents("/tmp/ac_script.sh", command, -1, NULL);
		g_spawn_command_line_async("gnome-terminal --hide-menubar -x sh /tmp/ac_script.sh", NULL);
	}
	else
	{
		command = g_strdup_printf("%s; rm -fr /tmp/ac_script.sh", gbuffer);
		g_file_set_contents("/tmp/ac_script.sh", command, -1, NULL);
...
void
show_standard_action(gchar *name)
{
	gchar *command;
	if (g_key_file_get_boolean(loaded_alarms, name, "ActionFirstSelected", NULL))
	{
		g_spawn_command_line_async("gnome-screensaver-command -l", NULL);
	}
	else
	{
		command = g_strdup("gnome-screensaver-command -a; xset dpms force off; rm -fr /tmp/ac_script.sh");
		g_file_set_contents("/tmp/ac_script.sh", command, -1, NULL);
		g_spawn_command_line_async("sh /tmp/ac_script.sh", NULL);
		g_free(command);
	}
...
void
stop_the_sound(GtkWidget *widget)
{
	gst_element_set_state (play, GST_STATE_NULL);
	gst_object_unref(play);
	gst_object_unref(bus);
