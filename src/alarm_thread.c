...
gulong
check_date(gint month, gint day, gint hour, gint minute, gint year)
{
	time_t rawtime;
	time(&rawtime);
	struct tm * mytime;
...
gboolean
check_date_excluded(gchar **ex, gint month, gint day)
{
	gint count = 0;

	while (TRUE)
	{
		if (ex[count] == NULL) break;
...
get_iso_date_from_scheduled(gchar *alarm_name)
{
	...
	gulong qdate = 0, alarm_time = g_key_file_get_integer(loaded_alarms, alarm_name, "DateTime", NULL);
	gint count = 0, hour, minute, year;
	gchar *sweekdays = g_key_file_get_string(loaded_alarms, alarm_name, "ScheduleWeekdays", NULL);
	gchar *smonths = g_key_file_get_string(loaded_alarms, alarm_name, "ScheduleMonths", NULL);
	gchar *sinclude = g_key_file_get_string(loaded_alarms, alarm_name, "ScheduleDateInclude", NULL);
	gchar *sexclude = g_key_file_get_string(loaded_alarms, alarm_name, "ScheduleDateExclude", NULL);
	...
	while (TRUE)
	{
		...
		{
			qdate = mktime(mytime);
			if (qdate > rawtime && !check_date_excluded(sexclude_split, mytime->tm_mon, mytime->tm_mday)) break;
		}
...
void
update_alarm_thread(gboolean thread)
{
	if (blocked) return;
	blocked = TRUE;
	glong diff = 0;
	...
	while(TRUE)
	{
		...
		{
			alarm_time = g_key_file_get_double(loaded_alarms, alarms[count], "DateTime", NULL);
			g_free(alarm_type);
		}
		...
		if (diff > 1)
		{
			if (snoozed)
			{
				...
				if (thread) gdk_threads_enter();
				gtk_list_store_set(store, &iter, 5, time_left, -1);
				if (thread) gdk_threads_leave();
				g_free(buf);
				g_free(time_left);
			}
			else
...
check_current_birthdays(void)
{
	...
	if (got_birthday)
	{
		buffer = g_strdup_printf(_("Today's birthdays: %s"), names->str);
