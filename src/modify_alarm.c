...
void fill_templates(GtkWidget * treeview)
{
	gtk_tree_view_set_model(GTK_TREE_VIEW(treeview), NULL);
	modify_templates_store =
	...
	GtkWidget *templates_notebook =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "notebook4"));
	GtkWidget *settings_notebook =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "notebook2"));

	GKeyFile *templates_key = g_key_file_new();

	g_key_file_load_from_file(templates_key, config_templates,
	...
	while (TRUE) {
		if (templates_content[templates_num] == NULL)
			break;
		buffer =
		    g_key_file_get_string(templates_key,
					  templates_content[templates_num],
					  "Title", NULL);
		gtk_list_store_append(GTK_LIST_STORE(modify_templates_store),
				      &iter);
		gtk_list_store_set(GTK_LIST_STORE(modify_templates_store),
				   &iter, 0, buffer, 1,
				   templates_content[templates_num], -1);
		g_free(buffer);
	...
	gtk_tree_view_set_model(GTK_TREE_VIEW(treeview),
				GTK_TREE_MODEL(modify_templates_store));
...
void notification_toggle(void)
{
	if (unload_boxes)
		return;
	GtkWidget *chk1 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "checkbutton1"));
	GtkWidget *chk2 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "checkbutton2"));
	GtkWidget *chk3 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "checkbutton3"));
	GtkWidget *chk4 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "checkbutton4"));
	GtkWidget *chk5 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "checkbutton5"));
	...
	if (!load)
		g_key_file_set_boolean(current_key, untitled_name,
				       "SoundEnabled", chk1_act);
	...
	if (!load)
		g_key_file_set_boolean(current_key, untitled_name,
				       "ActionEnabled", chk5_act);

}

void date_radio_toggle(void)
{
	GtkWidget *single_pref =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "single_day_button"));
	GtkWidget *schedule_pref =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "schedule_button"));
	GtkWidget *radio1 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "radiobutton1"));
	GtkWidget *radio2 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "radiobutton2"));
	GtkWidget *radio3 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "radiobutton3"));
	GtkWidget *radio4 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "radiobutton4"));
	...
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(radio3))) {
		...
		gtk_widget_set_sensitive(GTK_WIDGET(schedule_pref), FALSE);
		if (current_key != NULL)
			g_key_file_set_double(current_key, untitled_name,
					      "DateTime", current_time.tv_sec);
	}
	// Tomorrow
...
void set_time_format(void)
{
	...
	if (value == 24) {
		GtkWidget *hour =
		    GTK_WIDGET(gtk_builder_get_object(gxml, "hour_spin"));
...
void fill_default(void)
{
	...
	GtkWidget *title =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "alarm_title_entry"));
	...
	if (current_key != NULL)
		g_key_file_set_double(current_key, untitled_name, "DateTime",
				      current_time.tv_sec);
	g_key_file_set_string(current_key, untitled_name, "Title",
			      _("Untitled"));
	...
	g_key_file_set_string(current_key, untitled_name, "AlarmType",
			      "Single");
	g_key_file_set_boolean(current_key, untitled_name, "SoundEnabled",
			       TRUE);
	g_key_file_set_boolean(current_key, untitled_name, "CustomSoundEnabled",
			       FALSE);
	g_key_file_set_string(current_key, untitled_name, "CustomSoundFile",
			      "[DEFAULT]");
	g_key_file_set_boolean(current_key, untitled_name,
	...
	g_key_file_set_integer(current_key, untitled_name, "PassivePopupType",
			       0);
	g_key_file_set_integer(current_key, untitled_name,
			       "PassivePopupTimeout", 30);
	g_key_file_set_string(current_key, untitled_name, "PassivePopupTitle",
			      _("Popup"));
	g_key_file_set_string(current_key, untitled_name, "PassivePopupText",
			      _("Short popup"));
	g_key_file_set_boolean(current_key, untitled_name,
			       "DialogWindowEnabled", FALSE);
	g_key_file_set_string(current_key, untitled_name, "DialogTextColor",
			      "#000000000000");
	g_key_file_set_string(current_key, untitled_name,
	...
	g_key_file_set_string(current_key, untitled_name, "DialogTitle",
			      _("Dialog window"));
	g_key_file_set_string(current_key, untitled_name, "DialogMessage",
			      _("This is a small message"));
	g_key_file_set_boolean(current_key, untitled_name, "SnoozeEnabled",
			       FALSE);
	...
	g_key_file_set_string(current_key, untitled_name, "RunCommandContent",
			      "");
	g_key_file_set_boolean(current_key, untitled_name, "RunInTerminal",
			       TRUE);
	g_key_file_set_boolean(current_key, untitled_name, "ActionEnabled",
			       FALSE);
	g_key_file_set_boolean(current_key, untitled_name,
...
spin_change(GtkWidget * spinbutton,
	    GtkScrollType * scrolltype, gpointer user_data)
{
	glong datetime;
	struct tm *timeinfo;
	GKeyFile *keyfile = g_key_file_new();
	...
	GtkWidget *minute =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "minute_spin"));
	if (current_key != NULL)
		datetime =
		    g_key_file_get_double(current_key, untitled_name,
					  "DateTime", NULL);
	...
	timeinfo->tm_min =
	    gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(minute));
	if (current_key != NULL)
		g_key_file_set_double(current_key, untitled_name, "DateTime",
				      mktime(timeinfo));
...
void sound_props_check(void)
{
	...
	GtkWidget *custom1 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "custom_sound_check"));
	GtkWidget *custom2 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "custom_sound_button"));

	GtkWidget *constant1 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "constant_volume_radio"));
	GtkWidget *constant2 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "constant_volume_scale"));
	GtkWidget *constant3 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "constant_volume_label"));

	GtkWidget *faded1 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "faded_volume_radio"));
	GtkWidget *faded2 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "faded_volume_scale"));
	GtkWidget *faded3 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "faded_volume_label"));

	GtkWidget *repeat1 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "repeat_check"));
	GtkWidget *repeat2 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "repeat_scale"));
	GtkWidget *repeat3 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "repeat_label"));
...
{
	GtkWidget *sound =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "sound_prefs_dialog"));
	GtkWidget *custom1 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "custom_sound_check"));
	GtkWidget *custom2 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "custom_sound_button"));
	...
	if (buffer == NULL && bool_buffer == TRUE) {
		GtkWidget *message =
		    gtk_message_dialog_new_with_markup(GTK_WINDOW(sound),
	...
	if (buffer == NULL)
		buffer = g_strdup_printf("[DEFAULT]");
	...
	bool_buffer =
	    gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(constant1));

	g_key_file_set_boolean(current_key, untitled_name,
			       "ConstantSoundEnabled", bool_buffer);
...
void show_sound_properties(void)
{
	...
	bool_buffer =
	    g_key_file_get_boolean(current_key, untitled_name,
				   "CustomSoundEnabled", NULL);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(custom1), bool_buffer);
	buffer =
	    g_key_file_get_string(current_key, untitled_name, "CustomSoundFile",
				  NULL);
	...
	bool_buffer =
	    g_key_file_get_boolean(current_key, untitled_name,
				   "ConstantSoundEnabled", NULL);
	int_buffer =
	    g_key_file_get_integer(current_key, untitled_name,
				   "ConstantSoundValue", NULL);
	if (bool_buffer)
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(constant1),
					     TRUE);
	gtk_range_set_value(GTK_RANGE(constant2), int_buffer);

	bool_buffer =
	    g_key_file_get_boolean(current_key, untitled_name,
				   "FadeSoundEnabled", NULL);
	int_buffer =
	    g_key_file_get_integer(current_key, untitled_name, "FadeSoundValue",
				   NULL);
	if (bool_buffer)
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(faded1), TRUE);
	gtk_range_set_value(GTK_RANGE(faded2), int_buffer);

	bool_buffer =
	    g_key_file_get_boolean(current_key, untitled_name,
				   "RepeatSoundEnabled", NULL);
	int_buffer =
	    g_key_file_get_integer(current_key, untitled_name,
				   "RepeatSoundValue", NULL);
	if (bool_buffer)
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(repeat1), TRUE);
...
{
	GtkWidget *popup =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "popup_prefs_dialog"));
	GtkWidget *timeout =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "timeout_spin"));
	GtkWidget *title =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "title_entry"));
	GtkWidget *message =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "message_textview"));
	...
	integer_buffer =
	    gtk_combo_box_get_active(GTK_COMBO_BOX(priority_combo));
	g_key_file_set_integer(current_key, untitled_name, "PassivePopupType",
			       integer_buffer);
	...
	g_key_file_set_integer(current_key, untitled_name,
			       "PassivePopupTimeout", integer_buffer);
	...
	gbuffer =
	    gtk_text_buffer_get_text(GTK_TEXT_BUFFER(txt_buf), &start, &end,
				     TRUE);
	g_key_file_set_string(current_key, untitled_name, "PassivePopupText",
			      gbuffer);
...
void load_current_key()
{
	...
	GtkWidget *radio4 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "radiobutton4"));
	GtkWidget *calendar =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "calendar1"));
	...
	if (value == 12) {
		...
		if (ahour > 12) {
			gtk_spin_button_set_value(GTK_SPIN_BUTTON(hour),
						  ahour - 12);
	...
	gtk_calendar_select_month(GTK_CALENDAR(calendar), timeinfo->tm_mon,
	...
	if (g_strcmp0
	    (g_key_file_get_string
	     (buffer_key, untitled_name, "SoundEnabled", NULL), "true") == 0)
	...
	if (g_strcmp0
	    (g_key_file_get_string
	     (buffer_key, untitled_name, "PassivePopupEnabled", NULL),
	     "true") == 0)
	...
	if (g_strcmp0
	    (g_key_file_get_string
	     (buffer_key, untitled_name, "DialogWindowEnabled", NULL),
	     "true") == 0)
	...
	if (g_strcmp0
	    (g_key_file_get_string
	     (buffer_key, untitled_name, "RunCommand", NULL), "true") == 0)
...
{
	GtkWidget *dialog =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "add_alarm_dialog"));
	GtkWidget *tree =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "templates_tree"));
...
void translate_keys(GKeyFile * from, GKeyFile * to)
{
	...
	while (TRUE) {
		if (keys[count] == NULL)
			break;
		buffer =
		    g_key_file_get_value(from, untitled_name, keys[count],
					 NULL);
...
{
	GtkWidget *apply_template =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "apply_template_button"));
	GtkWidget *dialog =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "add_alarm_dialog"));
...
void modify_alarm_clicked_ok(void)
{
	...
	if (!gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(chk1)) &&
		...
		GtkWidget *error_dialog =
		    gtk_message_dialog_new(GTK_WINDOW(alarms_dialog),
					   GTK_DIALOG_DESTROY_WITH_PARENT |
					   GTK_DIALOG_MODAL,
					   GTK_MESSAGE_ERROR,
					   GTK_BUTTONS_CLOSE,
		...
		gtk_message_dialog_set_markup(GTK_MESSAGE_DIALOG(error_dialog),
					      _("<b>Error occurred!</b>"));
		gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG
							 (error_dialog),
							 _
							 ("You need to select at least one notification."));
	...
	if (editing_template == FALSE) {
		...
		gchar *buf1, *buf2;
		g_key_file_load_from_file(templates, config_templates,
					  G_KEY_FILE_NONE, NULL);
		if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(radio3))) {
			buf1 =
			    g_key_file_get_string(current_key, untitled_name,
...
void modify_alarm(gchar * name)
{
	...
	if (editing_template == FALSE) {
		...
		g_key_file_load_from_file(key, config_templates,
					  G_KEY_FILE_NONE, NULL);
	...
	buffer =
	    g_key_file_get_string(current_key, untitled_name, "AlarmType",
				  NULL);
...
void title_entry_change(GtkWidget * entry, gpointer user_data)
{
	const gchar *text = gtk_entry_get_text(GTK_ENTRY(entry));
	g_key_file_set_string(current_key, untitled_name, "Title", text);
}

void snooze_check_toggled(GtkWidget * snooze)
{
	gboolean active =
	    gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(snooze));
	GtkWidget *snooze_spin =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "snooze_spin"));
...
{
	GtkWidget *dialog =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "window_prefs_dialog"));
	GtkWidget *text_color_button =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "text_color_button"));
	GtkWidget *background_color_button =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "background_color_button"));
	GtkWidget *title_entry =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "dialog_title"));
	GtkWidget *message_entry =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "dialog_message"));
	GtkWidget *snooze_check =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "snooze_check"));
	GtkWidget *snooze_len =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "snooze_spin"));
	...
	tcolor =
	    g_key_file_get_string(current_key, untitled_name, "DialogTextColor",
				  NULL);
	gdk_color_parse(tcolor, &text_color);
	gtk_color_button_set_color(GTK_COLOR_BUTTON(text_color_button),
				   &text_color);
	...
	gtk_color_button_set_color(GTK_COLOR_BUTTON(background_color_button),
				   &background_color);

	text_size =
	    g_key_file_get_integer(current_key, untitled_name, "DialogTextSize",
				   NULL);
	gtk_combo_box_set_active(GTK_COMBO_BOX(text_size_combo), text_size);

	title =
	    g_key_file_get_string(current_key, untitled_name, "DialogTitle",
				  NULL);
	...
	message =
	    g_key_file_get_string(current_key, untitled_name, "DialogMessage",
				  NULL);
	gtk_text_buffer_set_text(GTK_TEXT_BUFFER(text_view_buffer), message,
				 -1);

	snooze_active =
	    g_key_file_get_boolean(current_key, untitled_name, "SnoozeEnabled",
				   NULL);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(snooze_check),
				     snooze_active);
	gtk_widget_set_sensitive(GTK_WIDGET(snooze_len), snooze_active);

	snooze_length =
	    g_key_file_get_integer(current_key, untitled_name, "SnoozeTime",
				   NULL);
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(snooze_len), snooze_length);

	gtk_dialog_run(GTK_DIALOG(dialog));

	gtk_color_button_get_color(GTK_COLOR_BUTTON(text_color_button),
				   &text_color);
	gtk_color_button_get_color(GTK_COLOR_BUTTON(background_color_button),
				   &background_color);
	...
	snooze_active =
	    gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(snooze_check));
	snooze_length =
	    gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(snooze_len));
	text_view_buffer =
	    gtk_text_view_get_buffer(GTK_TEXT_VIEW(message_entry));
	gtk_text_buffer_get_start_iter(GTK_TEXT_BUFFER(text_view_buffer),
				       &start_iter);
	gtk_text_buffer_get_end_iter(GTK_TEXT_BUFFER(text_view_buffer),
				     &end_iter);
	message =
	    gtk_text_buffer_get_text(GTK_TEXT_BUFFER(text_view_buffer),
				     &start_iter, &end_iter, TRUE);

	g_key_file_set_string(current_key, untitled_name, "DialogTextColor",
			      tcolor);
	g_key_file_set_string(current_key, untitled_name,
			      "DialogBackgroundColor", bcolor);

	g_key_file_set_integer(current_key, untitled_name, "DialogTextSize",
			       text_size);
	...
	g_key_file_set_boolean(current_key, untitled_name, "SnoozeEnabled",
			       snooze_active);
	g_key_file_set_integer(current_key, untitled_name, "SnoozeTime",
			       snooze_length);
...
{
	GtkWidget *dialog =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "command_prefs_dialog"));
	GtkWidget *run_script_entry =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "script_entry"));
	GtkWidget *run_in_terminal =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "command_terminal_check"));

	gchar *command;
	GtkTextIter start, end;
	gboolean run_in_terminal_checked;
	GtkTextBuffer *buffer =
	    gtk_text_view_get_buffer(GTK_TEXT_VIEW(run_script_entry));

	command =
	    g_key_file_get_string(current_key, untitled_name,
				  "RunCommandContent", NULL);
	run_in_terminal_checked =
	    g_key_file_get_boolean(current_key, untitled_name, "RunInTerminal",
				   NULL);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(run_in_terminal),
				     run_in_terminal_checked);

	gtk_text_buffer_set_text(GTK_TEXT_BUFFER(buffer), command,
				 strlen(command));
	...
	command =
	    gtk_text_buffer_get_text(GTK_TEXT_BUFFER(buffer), &start, &end,
				     TRUE);
	run_in_terminal_checked =
	    gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(run_in_terminal));
...
{
	GtkWidget *run_command_radio =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "radiobutton5"));
	GtkWidget *run_command_entry =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "command_entry"));
	GtkWidget *run_script_entry =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "script_entry"));
...
{
	GtkWidget *dialog =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "action_prefs_dialog"));
	GtkWidget *lock_monitor_radio =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "radiobutton7"));
	GtkWidget *turn_off_radio =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "radiobutton8"));
	...
	if (first_selected) {
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON
					     (lock_monitor_radio), TRUE);
	} else {
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(turn_off_radio),
					     TRUE);
	}
	...
	first_selected =
	    gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lock_monitor_radio));

	g_key_file_set_boolean(current_key, untitled_name,
			       "ActionFirstSelected", first_selected);

	gtk_widget_hide(GTK_WIDGET(dialog));
}

void remove_alarm(gchar * current)
{
	gchar *data;
...
void remove_alarm_dialog(void)
{
	...
	gchar *name =
	    g_key_file_get_string(loaded_alarms, current, "Title", NULL);
	gint result;
	...
	gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog),
						 _
						 ("Are you sure you want to remove alarm %s?"),
						 name);
...
gchar *check_modify_template_selected(void)
{
	...
	if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
		gtk_tree_model_get(GTK_TREE_MODEL(modify_templates_store),
				   &iter, 1, &name, -1);
		return name;
	} else
		return NULL;
...
void apply_current_template(void)
{
	...
	g_key_file_load_from_file(temp_key, config_templates, G_KEY_FILE_NONE,
				  NULL);
	...
	while (TRUE) {
		if (keys[count] == NULL)
			break;
		buffer =
		    g_key_file_get_value(temp_key, alarm, keys[count], NULL);
		g_key_file_set_value(current_key, untitled_name, keys[count],
				     buffer);
		g_free(buffer);
		count++;
	}
