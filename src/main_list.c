...
get_selected_path()
{
	...
	{
		gchar *path = gtk_tree_model_get_string_from_iter(GTK_TREE_MODEL(store), &iter);
...
main_list_popup (GtkWidget *tree, GdkEventButton *event, gpointer userdata)
{
	...
	GtkWidget *add_new_alarm = gtk_image_menu_item_new_with_mnemonic(_("_Add new alarm"));
	GtkWidget *add_new_alarm_image = gtk_image_new_from_stock("gtk-add", GTK_ICON_SIZE_MENU);
	gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(add_new_alarm), add_new_alarm_image);

	GtkWidget *add_new_counter = gtk_image_menu_item_new_with_mnemonic(_("_Add new counter"));
	GtkWidget *add_new_counter_image = gtk_image_new_from_stock("gtk-add", GTK_ICON_SIZE_MENU);
	gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(add_new_counter), add_new_counter_image);

	GtkWidget *menu_separator_1 = gtk_separator_menu_item_new();

	GtkWidget *remove_alarm = gtk_image_menu_item_new_with_mnemonic(_("_Remove this alarm"));
	GtkWidget *remove_alarm_image = gtk_image_new_from_stock("gtk-remove", GTK_ICON_SIZE_MENU);
	gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(remove_alarm), remove_alarm_image);

	GtkWidget *menu_separator_2 = gtk_separator_menu_item_new();

	GtkWidget *properties = gtk_image_menu_item_new_from_stock("gtk-properties", NULL);
	GtkWidget *menu_separator_3 = gtk_separator_menu_item_new();
	if (!show_menu)
	{
		show_menu_popup_menuitem = gtk_check_menu_item_new_with_mnemonic(_("_Show menu bar"));
	}
	...
	if (!show_menu)
	{
		...
		gtk_menu_shell_append(GTK_MENU_SHELL(menu), show_menu_popup_menuitem);

		g_signal_connect_after (G_OBJECT (show_menu_popup_menuitem), "activate",
...
void
create_list_model_and_view(void)
{
	...
	gtk_tree_view_column_add_attribute  (name_column, name_renderer, "markup", 0);
	gtk_tree_view_column_add_attribute  (name_column, name_renderer, "editable", 7);
	...
	gtk_tree_view_column_set_expand(GTK_TREE_VIEW_COLUMN(name_column), FALSE);
	...
	gtk_tree_view_column_add_attribute  (type_column, type_renderer, "pixbuf", 1);

	gtk_tree_view_column_add_attribute  (type_column, type_name_renderer, "markup", 2);

	gtk_tree_view_column_set_sizing (type_column, GTK_TREE_VIEW_COLUMN_FIXED);
	...
	gtk_tree_view_column_add_attribute  (date_column, date_renderer, "markup", 3);

	gtk_tree_view_column_pack_start (time_left_column, timeleft_renderer, FALSE);

	gtk_tree_view_column_add_attribute  (time_left_column, timeleft_renderer, "markup", 5);

	gtk_tree_view_column_set_sizing (time_left_column, GTK_TREE_VIEW_COLUMN_FIXED);
	gtk_tree_view_column_set_fixed_width (time_left_column, 100);

	gtk_tree_view_column_set_title (GTK_TREE_VIEW_COLUMN(name_column), _("Name"));
	gtk_tree_view_column_set_title (GTK_TREE_VIEW_COLUMN(type_column), _("Alarm type"));
	gtk_tree_view_column_set_title (GTK_TREE_VIEW_COLUMN(date_column), _("Date and time"));
	gtk_tree_view_column_set_title (GTK_TREE_VIEW_COLUMN(time_left_column), _("Time left"));
...
void
update_list_entries(void)
{
	GtkTreeSelection *selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(main_tree));
	...
	gint timetype = g_key_file_get_integer(keyfile, "Global", "TimeType", NULL);

	GTimeVal current_time;
	struct tm * timeinfo;
	timeinfo = localtime ((const time_t *)&current_time );

	gchar *title = "", *gtime = "", *ghour = "", *gminute = "", *gmonth = "", *gday = "", *gdate = "", *buffer = "", *to_set = "";
	GtkWidget *note = GTK_WIDGET (gtk_builder_get_object (gxml, "main_notebook"));
	GtkWidget *alarm_test_toolbutton = GTK_WIDGET (gtk_builder_get_object (gxml, "alarm_test_toolbutton"));
	GtkWidget *alarm_properties_toolbutton = GTK_WIDGET (gtk_builder_get_object (gxml, "alarm_properties_toolbutton"));
	GtkWidget *remove_alarm_toolbutton = GTK_WIDGET (gtk_builder_get_object (gxml, "remove_alarm_toolbutton"));
	GtkWidget *remove_alarm_menu = GTK_WIDGET (gtk_builder_get_object (gxml, "remove_alarm_menuitem"));
	gboolean snoozed;
	gchar *snoozed_old;
	...
	if (number_of_groups == 0)
	{
		...
		gtk_widget_set_sensitive(GTK_WIDGET(alarm_test_toolbutton), FALSE);
		gtk_widget_set_sensitive(GTK_WIDGET(alarm_properties_toolbutton), FALSE);
		gtk_widget_set_sensitive(GTK_WIDGET(remove_alarm_toolbutton), FALSE);
	...
	else
	{
		...
		gtk_widget_set_sensitive(GTK_WIDGET(alarm_test_toolbutton), TRUE);
		gtk_widget_set_sensitive(GTK_WIDGET(alarm_properties_toolbutton), TRUE);
		gtk_widget_set_sensitive(GTK_WIDGET(remove_alarm_toolbutton), TRUE);
		gtk_widget_set_sensitive(GTK_WIDGET(remove_alarm_menu), TRUE);
	}

	for (count = 0; count < number_of_groups; count++)
	{
		datetime = g_key_file_get_double(loaded_alarms, groups[count], "DateTime", NULL);
		snoozed = g_key_file_get_boolean(loaded_alarms, groups[count], "Snoozed", NULL);
		snoozed_old = g_key_file_get_string(loaded_alarms, groups[count], "SnoozedOldTime", NULL);
		...
		if (snoozed)
		{
			...
			{
				if (hour < 12 && hour > 0) {
						gtime = g_strdup_printf("<i><b>%s:%s %s</b>\n<small>%s</small></i>", ghour, gminute, _("AM"), gdate);
					}

				if (hour > 12)	{
						gtime = g_strdup_printf("<i><b>%i:%s %s</b>\n<small>%s</small></i>", hour - 12, gminute, _("PM"),  gdate);
					}

				if (hour == 12) {
						gtime = g_strdup_printf("<i><b>%i:%s %s</b>\n<small>%s</small></i>", 12, gminute, _("PM"), gdate);
					}
				if (hour == 24) {
						gtime = g_strdup_printf("<i><b>%i:%s %s</b>\n<small>%s</small></i>", 12, gminute, _("AM"), gdate);
					}
			...
			{
				gtime = g_strdup_printf("<i><b>%s:%s</b>\n<small>%s</small></i>", ghour, gminute, gdate);
			}
		...
		else
		{
			...
			{
				if (hour < 12 && hour > 0) {
						gtime = g_strdup_printf("<b>%s:%s %s</b>\n<small>%s</small>", ghour, gminute, _("AM"), gdate);
					}

				if (hour > 12)	{
						gtime = g_strdup_printf("<b>%i:%s %s</b>\n<small>%s</small>", hour - 12, gminute, _("PM"), gdate);
					}

				if (hour == 12) {
						gtime = g_strdup_printf("<b>%i:%s %s</b>\n<small>%s</small>", 12, gminute, _("PM"), gdate);
					}
				if (hour == 24) {
						gtime = g_strdup_printf("<b>%i:%s %s</b>\n<small>%s</small>", 12, gminute, _("AM"), gdate);
					}
			...
			{
				gtime = g_strdup_printf("<b>%s:%s</b>\n<small>%s</small>", ghour, gminute, gdate);
			}
		}


		buffer = g_key_file_get_string(loaded_alarms, groups[count], "AlarmType", NULL);



		if (g_strcmp0(buffer, "Single") == 0)
		{
			if (snoozed) to_set = g_strdup(_("<i>Single</i>"));
			else to_set = g_strdup(_("Single"));
		}
		if (g_strcmp0(buffer, "Counter") == 0)
		{
			if (snoozed) to_set = g_strdup(_("<i>Counter</i>"));
			else to_set = g_strdup(_("Counter"));
		}
		if (g_strcmp0(buffer, "Schedule") == 0)
		{
			if (snoozed) to_set = g_strdup(_("<i>Scheduled</i>"));
			else to_set = g_strdup(_("Scheduled"));
		}

		if (snoozed)
		{
			gchar *buf = g_key_file_get_string(loaded_alarms, groups[count], "Title", NULL);
			title = g_strdup_printf("<i>%s\n<small>Snoozed</small></i>", buf);
		}
		else
		{
			title = g_key_file_get_string(loaded_alarms, groups[count], "Title", NULL);
		}

	gtk_list_store_append(GTK_LIST_STORE(store), &iter);
	gtk_list_store_set(store, &iter, 0, title, 2, to_set, 3, gtime, 4, groups[count], 6, count, 7, FALSE, -1);
	}
	...
	if (count > 0 && check_current_selected() == NULL)
	{
		...
		gtk_tree_selection_select_path (GTK_TREE_SELECTION(selection), zero_path);

	}
	if (path_selected < 0) return;
...
change_selection(void)
{
	...
	GtkWidget *remove = GTK_WIDGET(gtk_builder_get_object(gxml, "remove_alarm_toolbutton"));
	GtkWidget *remove_menu = GTK_WIDGET(gtk_builder_get_object(gxml, "remove_alarm_menuitem"));
	GtkWidget *props = GTK_WIDGET(gtk_builder_get_object(gxml, "alarm_properties_toolbutton"));
	GtkWidget *test = GTK_WIDGET(gtk_builder_get_object(gxml, "alarm_test_toolbutton"));
