...
{
	GtkWidget *templates_notebook = GTK_WIDGET (gtk_builder_get_object (gxml, "templates_notebook"));
	...
	while (TRUE)
	{
		...
		buffer = g_key_file_get_string(templates, templates_content[templates_num], "Title", NULL);
...
void
reload_birthdays(void)
{
	GtkWidget *birthdays_notebook = GTK_WIDGET (gtk_builder_get_object (gxml, "birthdays_notebook"));
	...
	while (TRUE)
	{
		...
		name = g_key_file_get_string(birthdays, birthdays_content[birthdays_num], "Name", NULL);
		date = g_key_file_get_string(birthdays, birthdays_content[birthdays_num], "Date", NULL);
		gtk_list_store_append(GTK_LIST_STORE(birthdays_store), &iter);
		gtk_list_store_set(GTK_LIST_STORE(birthdays_store), &iter, 0, name, 1, date, 2, birthdays_content[birthdays_num], -1);
		g_free(name);
		g_free(date);
		birthdays_num++;
	}
...
add_birthday_ok(void)
{
	...
	g_key_file_load_from_file(birth_key, config_birthdays, G_KEY_FILE_NONE, NULL);
...
check_birthday_selected(void)
{
	...
	{
		gtk_tree_model_get (GTK_TREE_MODEL(birthdays_store), &iter, 2, &name, -1);
	    return name;
	}
	else
		return NULL;
...
check_template_selected(void)
{
	...
	if (gtk_tree_selection_get_selected(selection, &model, &iter))
	{
		gtk_tree_model_get (GTK_TREE_MODEL(templates_store), &iter, 1, &name, -1);
	    return name;
	}
	else
		return NULL;
...
show_templates(void)
{
	GtkWidget *dialog = GTK_WIDGET (gtk_builder_get_object (gxml, "templates_birthdays_dialog"));

	GtkWidget *templates_event = GTK_WIDGET (gtk_builder_get_object (gxml, "templates_event"));
	GtkWidget *birthdays_event = GTK_WIDGET (gtk_builder_get_object (gxml, "birthdays_event"));
	templates_tree = GTK_WIDGET (gtk_builder_get_object (gxml, "templates_tre"));
