...
void show_preferences(void)
{
	...
	GtkWidget *start_automatically =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "start_automatically"));
	GtkWidget *start_minimized =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "start_minimized"));
	GtkWidget *radio_12 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "radio_12"));
	GtkWidget *radio_24 =
	    GTK_WIDGET(gtk_builder_get_object(gxml, "radio_24"));
	gchar *keydata, *desktop_file, *autostart_file;
	gboolean buffer;
	gchar *auto_path =
	    g_strdup_printf("%s/.config/autostart", g_getenv("HOME"));

	GKeyFile *config_key = g_key_file_new();

	g_key_file_load_from_file(config_key, config_global, G_KEY_FILE_NONE,
				  0);
	...
	autostart_file =
	    g_strdup_printf("%s/.config/autostart/alarm-clock.desktop",
			    g_getenv("HOME"));

	if (g_key_file_get_boolean
	    (config_key, "Global", "StartMinimized", NULL))
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(start_minimized),
					     TRUE);
	else
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(start_minimized),
					     FALSE);
	...
	if (g_key_file_get_integer(config_key, "Global", "TimeType", NULL) ==
	    12)
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(radio_12), TRUE);
	else if (g_key_file_get_integer(config_key, "Global", "TimeType", NULL)
		 == 24)
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(radio_24), TRUE);
	else {
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(radio_12),
					     FALSE);
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(radio_24),
					     FALSE);
	}
	...
	buffer =
	    gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(start_minimized));
